...
class StripedHyena(nn.Module):
    def __init__(self, config):
        super().__init__()
        # 👇 Keep all original initialization exactly the same
        self.config = config
    ...
    def forward(self, x, inference_params_dict=None, padding_mask=None, 
        ...
        """
        🔧 ENHANCED: Same as original but with optional hidden state extraction
        
        When output_hidden_states=False and return_dict=False (default):
        - Behaves EXACTLY like the original
        - Returns: (logits, inference_params_dict_out)
        
        When output_hidden_states=True or return_dict=True:
        - Collects hidden states from all layers
        - Returns: dict or (logits, hidden_states)
        """
        L = x.shape[1]
        x = self.embedding_layer.embed(x)
        
        # 🔧 NEW: Only create containers when needed (backward compatible)
        ...
        if inference_params_dict is not None:
            x, inference_params_dict_out = self.stateful_forward(
                ...
                # 🔧 NEW: Pass hidden state collection info (ignored if not needed)
                all_hidden_states=all_hidden_states
            )
        else:
            x, inference_params_dict_out = self.stateless_forward(
                x, 
                padding_mask=padding_mask,  # ← EXACT SAME AS ORIGINAL
                # 🔧 NEW: Pass hidden state collection info (ignored if not needed)
                all_hidden_states=all_hidden_states
            )

        # # 🔧 NEW: Save final hidden state before norm (only if needed)
        # last_hidden_state = x.clone() if return_dict else None

        # 👇 PRESERVE ORIGINAL FINAL PROCESSING EXACTLY
        x = self.norm(x)
        last_hidden_state = x.clone() if return_dict else None
        x = self.unembed.unembed(x)
# 🔧 NEW: Save final hidden state AFTER norm (only if needed)
        

        # 🔧 NEW: Return format (backward compatible)
        if return_dict:
            return {
                'logits': x,
    ...
    def stateful_forward(self, x, inference_params_dict=None, all_hidden_states=None):
        ...
        for block_idx, block in enumerate(self.blocks):
            ...
            #  NEW: Collect hidden state if requested (doesn't affect original logic)
    ...
    # 🔧 NEW: Convenience methods (don't affect original functionality)
    def extract_layer_hidden_states(self, input_ids, target_layer, padding_mask=None):
        """Extract hidden states from a specific layer"""
        if not (0 <= target_layer < len(self.blocks)):
            raise ValueError(f"target_layer {target_layer} out of range [0, {len(self.blocks)-1}]")
        
        with torch.no_grad():
            outputs = self.forward(input_ids, padding_mask=padding_mask, 
            ...
            if target_layer < len(outputs['hidden_states']):
                return {
                    ...
                    'shape': outputs['hidden_states'][target_layer].shape
                }
            else:
                raise IndexError(f"Layer {target_layer} not found")

    def extract_multiple_layers(self, input_ids, layer_list, padding_mask=None):
        """Extract hidden states from multiple layers"""
        for layer in layer_list:
            if not (0 <= layer < len(self.blocks)):
                raise ValueError(f"Layer {layer} out of range [0, {len(self.blocks)-1}]")
        ...
        with torch.no_grad():
            ...
            for layer in layer_list:
                if layer < len(outputs['hidden_states']):
                    result[layer] = outputs['hidden_states'][layer]
                else:
                    result[layer] = None
            return result
    ...
    def precompute_filters(self, L, device):
        """
        Safer precompute:
        - Compute residues/poles exponentials on CPU in FP32 (numerically stable).
        - Use log-exp trick: exp(log(poles) * t) instead of (poles ** t).
        - Move only the final h buffer to GPU in BF16 to cut VRAM in half.
        - Honors long_fir_threshold if set; otherwise uses provided L.
        """
        import torch
        from torch import no_grad

        # Prefer compact storage for h on GPU
        STORE_DTYPE = torch.bfloat16

        def _ensure_time(filter_mod, L_eff, dev_hint):
            # Support either update_time or _set_time
            if hasattr(filter_mod, "update_time"):
                # we'll compute on CPU, but keep the internal t on CPU for this precompute
                filter_mod.update_time(L_eff, "cpu")
            elif hasattr(filter_mod, "_set_time"):
                # _set_time may assume the device you pass; keep it CPU for CPU compute
                filter_mod._set_time(L_eff, "cpu")
            else:
                raise AttributeError("ParallelHyenaFilter missing update_time/_set_time")

        for block_idx, block in enumerate(self.blocks):
            # Only apply to Hyena blocks
            if not isinstance(block, ParallelGatedConvBlock):
                continue
            if not isinstance(block.filter, ParallelHyenaFilter):
                continue

            # Effective length: respect long_fir_threshold if provided
            L_eff = block.filter.long_fir_threshold or L
            print_rank_0(f"[safe precompute] block={block_idx} L={L_eff}")

            # Prepare time vector on CPU
            _ensure_time(block.filter, L_eff, device)

            with no_grad():
                # --- Compute on CPU, FP32 ---
                # Move params to CPU FP32 as complex
                residues = torch.view_as_complex(block.filter.residues.detach().to(dtype=torch.float32, device="cpu"))
                poles    = torch.view_as_complex(block.filter.poles.detach().to(dtype=torch.float32, device="cpu"))

                # Time vector on CPU FP32
                t = block.filter.t.to(device="cpu", dtype=torch.float32)  # shape [1,1,L]

                # log-exp trick for stability: exp(log(poles) * t) instead of poles**t
                logp = poles.log()                            # [num_sys, state, 1]
                # Broadcast over time; result real-valued after sum over state dim
                h_cpu = (residues * (logp * t).exp()).real.sum(1, keepdim=False)[None]  # [1, num_sys, L]

                # If grouped Hyena is used, repeat h per group to match hidden layout
                if getattr(block.filter, "hyena_filter_groups", 1) > 1:
                    num_groups = block.filter.hyena_filter_groups
                    hidden_size = block.filter.hidden_size
                    num_sys = hidden_size // num_groups
                    # repeat_interleave along num_sys axis to fill hidden_size
                    h_cpu = h_cpu.repeat_interleave(num_sys, dim=1)  # [1, hidden_size, L]

                # --- Store compact on GPU ---
                block.filter.h = h_cpu.to(device=device, dtype=STORE_DTYPE, non_blocking=True)

                # Cleanup host temporaries early
                del residues, poles, logp, t, h_cpu

            # Optional: help the allocator between blocks
