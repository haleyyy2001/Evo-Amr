...
## Evo-AMR Modifications

This directory contains the official Evo-1-131k-base model files alongside
**`modified_model.py`**, a backward-compatible extension of `StripedHyena`
developed for the Evo-AMR thesis project (H. Tai, Columbia, 2025).

Changes in `modified_model.py` relative to the original `model.py`:

- `output_hidden_states: bool` and `return_dict: bool` flags added to `forward()`.
  Enables collecting all 32 block hidden states in a single forward pass without
  breaking the original return signature.
- `extract_layer_hidden_states(input_ids, target_layer)` — extract hidden states
  from a specific block (0-indexed). Used to probe layer-wise representations.
- `extract_multiple_layers(input_ids, layer_list)` — batch extraction for
  systematic layer diagnostic sweeps across all layers.

All changes are strictly backward-compatible. Loading the original checkpoint and
calling `model(x)` without the new flags produces identical output to the
original `model.py`.
